...
def runGame(i_gamePath, i_fileToRun = None, i_gameInfo = None):
    ...
    #atari800GemusScript(utils.joinPaths(tempDirPath, gameFiles)[0], i_gameInfo, i_gameInfo["Gemus"])
    #return
...
def atari800GemusScript(i_gameFilePath, i_gameInfo, i_gemusText):
    ...
    # If games was written in the PILOT language (I'm Different!),
    # insert the PILOT ROM cartridge
    if gemus.fieldContains("pilotrom", "yes"):
        args += ["-cart", "/mnt/gear/games/Atari 800/gamebases/Atari 800 v12/Emulators/Atari800Win4-1/PILOT.rom", i_gameFilePath]
    # Depending on file extension, boot programs, tapes, carts and disks with appropriate options,
    # and in the tape case also maybe turn BASIC off
    ...
    args = ["atari800"] + args
    print(args)
    utils.shellStartTask(args)

    if gemus.fieldContains("fullscreen", ["true", "on", "yes", "1"]):
        args += ["-fullscreen"]
    elif gemus.fieldContains("fullscreen", ["false", "off", "no", "0"]):
        args += ["-windowed"]
    ...
    # Hardware settings
    if gemus.fieldContains("model", ["osa", "osb", "xl", "ramboxe"]):
        if gemus.fieldContains("model", "osa"):
            Set_CFG_Value("%emupath%\GB800.TXT", "MACHINE_TYPE", "Atari OS/A")
        elif gemus.fieldContains("model", "osb"):
            Set_CFG_Value("%emupath%\GB800.TXT", "MACHINE_TYPE", "Atari OS/B")
        elif gemus.fieldContains("model", "xl"):
            Set_CFG_Value("%emupath%\GB800.TXT", "MACHINE_TYPE", "Atari XL/XE")
        elif gemus.fieldContains("model", "ramboxe"):
            Set_CFG_Value("%emupath%\GB800.TXT", "MACHINE_TYPE", "Atari XL/XE")
            Set_CFG_Value("%emupath%\GB800.TXT", "RAM_SIZE", "320 (RAMBO)")
    else:
        if gemus.fieldNotEmpty("model"):
            Show_Message("Invalid value for key=value pair MODEL entered.\nPossible values are:\nosa, osb, xl, ramboxe\nDefault xl is used.")
        Set_CFG_Value("%emupath%\GB800.TXT", "RAM_SIZE", "64")
        Set_CFG_Value("%emupath%\GB800.TXT", "MACHINE_TYPE", "Atari XL/XE")

    if gemus.fieldIf Key_emuos CONTAINS(true||on||yes||1)
    Add_CLP( -emuos)
